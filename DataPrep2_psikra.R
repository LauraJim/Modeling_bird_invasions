...
#########################  
###EXAMPLE WITH psikra### (Psittacula krameri)
######################### 
...
#select psikra
native.bird <- native.occs[native.occs$species=="psikra",]
  nrow(native.bird)
invasive.bird <- invasive.occs[invasive.occs$species=="psikra",]  
  nrow(invasive.bird)  

#restrict GBIF data to occurrences in (close to native range boundaries)
native.range.shp <- readOGR("E:/SDM/RANGES/psikra","psikra")
