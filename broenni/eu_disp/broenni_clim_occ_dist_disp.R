...
bird.species <- list.dirs(path = "E:/SDM/Modeling_bird_invasions/ranges", full.names = FALSE, recursive = FALSE)
...
############################################################################  
regions <- readOGR("E:/SDM/Modeling_bird_invasions/bioreg_occ_dist_climgrids/backgrounds",species.id)
#europe <- readOGR("E:/SDM/Modeling_bird_invasions/biogeo","europe")
  
##occurrence data
native.occs <- readOGR("E:/SDM/Modeling_bird_invasions/occurrences","native.range")
invasive.occs <- readOGR("E:/SDM/Modeling_bird_invasions/occurrences","invasive.range")
...
native.range.shp <- readOGR(paste0("E:/SDM/Modeling_bird_invasions/ranges/",species.id),species.id)
...
######################################################
invasive.range.shp <- readOGR(paste("E:/SDM/Modeling_bird_invasions/ranges/",species.id,sep=""),paste("for_",species.id,"_europe_inv",sep=""))
...
################################
  ...
  #setwd("E:/SDM/Modeling_bird_invasionsbroenni")
  #dir.create(species.id)
  
  #mypath <- file.path("E:/SDM/Modeling_bird_invasionsbroenni",species.id,paste("native range occurrende density", species.id, ".png", sep = ""))
  ...
  #mypath <- file.path("E:/SDM/Modeling_bird_invasionsbroenni",species.id,paste("invasive range occurrence density", species.id, ".png", sep = ""))
  #png(file=mypath, width = 225, height = 225, units='mm', res = 300)
  #ecospat.plot.niche(z2,title="PCA-env - NA niche",name.axis1="PC1",name.axis2="PC2")
  #dev.off()
  
  # contribution of original variables
  #mypath <- file.path("E:/SDM/Modeling_bird_invasionsbroenni",species.id,paste("PCA contributions", species.id, ".png", sep = ""))
  #png(file=mypath, width = 225, height = 225, units='mm', res = 300)
  ...
  #mypath <- file.path("E:/SDM/Modeling_bird_invasionsbroenni",species.id,paste("niche overlap native range density", species.id, ".png", sep = ""))
  #png(file=mypath, width = 225, height = 225, units='mm', res = 300)
  #ecospat.plot.niche.dyn(z1,z2,quant=0.5,name.axis1="PC1",name.axis2="PC2",interest=1)
  #dev.off()
  
  #mypath <- file.path("E:/SDM/Modeling_bird_invasionsbroenni",species.id,paste("niche overlap invasive range density", species.id, ".png", sep = ""))
  #png(file=mypath, width = 225, height = 225, units='mm', res = 300)
