...
#data.niche_th_sp <- read.table("E:/SDM/broenni/eu_disp/niche_results_th_sp.txt",sep="\t",h=T)
#  head(data.niche_th_sp)
  
data.niche_th_env <- read.table("E:/SDM/broenni/eu_disp/niche_results_th_env.txt",sep="\t",h=T)
...
niche.dyn.stab_vio <- ggplot(data.niche_th_env, aes(x=as.factor(threshold), y=stability.index)) + geom_violin()  
  ...
  ggsave(filename="Fig_Revision_X1b.png",
         plot=test,
         device='tiff',
         width=297,
         height=210,
...
niche.summary <- data.niche_th_env.75 %>%summarise(niche_overlap_F.D.median = median(niche_overlap_F.D),
                                                     niche_overlap_F.D.sd = sd(niche_overlap_F.D),
