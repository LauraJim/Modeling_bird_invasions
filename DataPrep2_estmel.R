...
#climate grids (5mins)
datafiles <- list.files(path="./worldclimv2", 
...
#########################  
###EXAMPLE WITH estmel### (Estrilda melpoda)
######################### 
...
#select estmel
native.bird <- native.occs[native.occs$species=="estmel",]
  nrow(native.bird)
invasive.bird <- invasive.occs[invasive.occs$species=="estmel",]  
  nrow(invasive.bird)  

#restrict GBIF data to occurrences in (close to native range boundaries)
native.range.shp <- readOGR("./ranges/estmel","estmel")
