...
# COMPUTE PA (PRESENCE/ABSENCE) MODELS ####
  ...
  #for.myimon2022 <- na.omit(as.data.frame(gbif_data$data)[c(2:4)])
  #for.myimon2022 <- for.myimon2022[c(1,3,2)]
  #colnames(for.myimon2022) <- c("sp","x","y")
  #for.myimon2022$sp <- rep("myimon",nrow(for.myimon2022))
  #head(for.myimon2022)
  #myimon2022<-humboldt.occ.rarefy(in.pts=for.myimon2022,colxy=2:3, rarefy.dist = 100, rarefy.units = "km")
  #plot(myimon2022$x,myimon2022$y)
  #write.table(myimon2022,"myimon2022.txt",sep="\t",row.names=FALSE)
  #myimon2022 <- read.table("myimon2022.txt",sep="\t",h=T)
  #head(myimon2022)
  #plot(myimon2022$x,myimon2022$y)
  
  #myimon2022 <- occ_nat
  #myimon2022 <- myimon2022[c(2:3)]
  #colnames(myimon2022) <- c("x","y")
  myimon.points <- SpatialPointsDataFrame(coords = c(occ_nat[,c("lon", "lat")]),
                                          proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"),
                                          data = occ_nat)
  #get native range BirdLife   
  birdlife.raw<-readOGR("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/ranges/myimon","myimon")
  birdlife.raw <- birdlife.raw[birdlife.raw$ORIGIN==1,]
  birdlife <- fortify(birdlife.raw)
  
  myimon.points <- spTransform(myimon.points, CRS(proj4string(birdlife.raw)))
  myimon.points <- raster::intersect(myimon.points,birdlife.raw)
  plot(myimon.points)
  head(myimon.points)
  myimon.points.spatV <- vect(myimon.points)
  ...
  #extract data
  temp1 <- terra::extract(climgrids_nat,myimon.points.spatV,xy=TRUE)
...
forPC1 <- data.frame(seq(min(data.frame(PC1)),max(data.frame(PC1)),by=0.1))
  colnames(forPC1)<- "PC1"
  forPC1$PC2 <- rep(mean(PC2$myimon_ClimGridsPCA.native_2))
  head(forPC1)
  #GLM
...
#
  ...
  colnames(forPC2)<- "PC2"
  forPC2$PC1 <- rep(mean(PC1$myimon_ClimGridsPCA.native_1))
  head(forPC2)
  summary(forPC2)
  #GLM
...
#read in occurrence data
  occurrences <- read.csv(paste0("../../Modeling_bird_invasions/Nf_modeling/occurrences-v3/myimon_invasive.csv"))
...
###NicheMapperSpecies
#    
nm_species <- raster(paste0("../Modeling_bird_invasions/NicheMapper/","myimon_yearly_energetics_pa.tif"))    
...
###################################################################################################################################    
  #get background map
  nativemap <- rast("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/ranges/myimon/myimon_ClimGridsPCA.native.tif")
...
ggsave(filename="myimon_MultiPanel.tiff",
         plot=p.final,
         device='tiff',
         width=89,
         height=89*2,
