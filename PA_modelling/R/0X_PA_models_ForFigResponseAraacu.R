...
# COMPUTE PA (PRESENCE/ABSENCE) MODELS ####
  ...
  #get BirdLife data
  myspecies <- c("Thectocercus acuticaudatus")
  #gbif_data <- occ_data(scientificName = myspecies, hasCoordinate = TRUE, limit = 30000)
  #for.araacu2022 <- na.omit(as.data.frame(gbif_data$data)[c(2:4)])
  #for.araacu2022 <- for.araacu2022[c(1,3,2)]
  #colnames(for.araacu2022) <- c("sp","x","y")
  #for.araacu2022$sp <- rep("araacu",nrow(for.araacu2022))
  #head(for.araacu2022)
  #araacu2022<-humboldt.occ.rarefy(in.pts=for.araacu2022,colxy=2:3, rarefy.dist = 100, rarefy.units = "km")
  #plot(araacu2022$x,araacu2022$y)
  #write.table(araacu2022,"araacu2022.txt",sep="\t",row.names=FALSE)
  araacu2022 <- read.table("araacu2022.txt",sep="\t",h=T)
  head(araacu2022)
  plot(araacu2022$x,araacu2022$y)
  
  araacu.points <- SpatialPointsDataFrame(coords = c(araacu2022[,c("x", "y")]),
                                          proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"),
                                          data = araacu2022)
  #get native range BirdLife   
  birdlife.raw<-readOGR("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/ranges/araacu","araacu")
  birdlife <- fortify(birdlife.raw)
  
  araacu.points <- spTransform(araacu.points, CRS(proj4string(birdlife.raw)))
  araacu.points <- raster::intersect(araacu.points,birdlife.raw)
  plot(araacu.points)
  head(araacu.points)
  araacu.points.spatV <- vect(araacu.points)
  ...
  #extract data
  temp1 <- terra::extract(climgrids_nat,araacu.points.spatV,xy=TRUE)
...
forPC1 <- data.frame(seq(min(data.frame(PC1)),max(data.frame(PC1)),by=0.1))
  colnames(forPC1)<- "PC1"
  forPC1$PC2 <- rep(mean(PC2$araacu_ClimGridsPCA.native_2))
  head(forPC1)
  #GLM
...
#
  ...
  colnames(forPC2)<- "PC2"
  forPC2$PC1 <- rep(mean(PC1$araacu_ClimGridsPCA.native_1))
  head(forPC2)
  summary(forPC2)
  #GLM
...
#read in occurrence data
  occurrences <- read.csv(paste0("../../Modeling_bird_invasions/Nf_modeling/occurrences-v3/araacu_invasive.csv"))
...
thresholds5E_glm <- tresholds5E[c(1:2)]
  ...
  p_glm5E <- ggplot() + geom_raster(data=glm_richness_5E_df,aes(x=x,y=y,fill=probs))+theme_void()+scale_fill_manual(values = colors1)
  p_glm5E <- p_glm5E + theme(legend.position = "none")
  p_glm5E <- p_glm5E + geom_point(data=occurrences[c(2:3)],aes(x=lon,y=lat),colour="red",size=0.8)
  p_glm5E
...
thresholds5E_bart <- tresholds5E[c(1,3)]
  ...
  p_bart5E <- ggplot() + geom_raster(data=bart_richness_5E_df,aes(x=x,y=y,fill=probs))+theme_void()+scale_fill_manual(values = colors1)
    p_bart5E <- p_bart5E + theme(legend.position = "none")
    p_bart5E <- p_bart5E + geom_point(data=occurrences[c(2:3)],aes(x=lon,y=lat),colour="red",size=0.8)
    p_bart5E
...
thresholds5E_fne <- tresholds5E[c(1:4)]
  ...
  p_fne5E <- ggplot() + geom_raster(data=fne_richness_5E_df,aes(x=x,y=y,fill=probs))+theme_void()+scale_fill_manual(values = colors1)
    ...
    p_fne5E <- p_fne5E + geom_point(data=occurrences[c(2:3)],aes(x=lon,y=lat),colour="red",size=0.8)
    p_fne5E
    
#    
###NicheMapperSpecies
#    
nm_species <- raster(paste0("../../Modeling_bird_invasions/NicheMapper/","ARAACU_yearly_energetics_pa.tif"))    
  ...
  NM_fig <- ggplot() + geom_raster(data=NM_points_df,aes(x=x,y=y,fill=probs))+theme_void()+scale_fill_manual(values = colors1)
    ...
    NM_fig <- NM_fig + geom_point(data=occurrences[c(2:3)],aes(x=lon,y=lat),colour="red",size=0.8)
    NM_fig
...
###################################################################################################################################    
# Read functions
setwd("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions")   
...
###################################################################################################################################    
  #get background map
  nativemap <- rast("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/ranges/araacu/nativemap.tif")
...
native.map <- ggplot() + geom_raster(data=native.map_points,aes(x=x,y=y,fill=values))+theme_void()+scale_fill_manual(values = "#fff333")
  ...
  native.map <- native.map + geom_point(data=sp.occ.nat[c(1:2)],aes(x=lon,y=lat),colour="blue",size=0.3,shape=19)
...
p.final <- ggarrange(p1,p2,p34,nrow=3,heights = c(1.5, 1,1))+ geom_subview(x=.40, y=.72, subview=leg)
  ...
  p.final <- p.final + annotate("text", label = "sensitivity: 0.73", x = 0.125, y = 0.97,size=2)
  p.final <- p.final + annotate("text", label = "sensitivity: 0.36", x = 0.625, y = 0.97,size=2)
  p.final <- p.final + annotate("text", label = "sensitivity: 0.73", x = 0.12, y = 0.54,size=2)
