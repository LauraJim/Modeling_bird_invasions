...
dir.create("../DS/2.5E/DS_inv_region_tables/")
dir.create("../DS/2.5E/DS_MESS_JPGs")
...
for (species in species_list) {
  ...
  write.csv(dat_inv, paste0("../DS/2.5E/DS_inv_region_tables/", species, "_inv_region_table.csv"), row.names = FALSE)
...
for (species in species_list) {
  ...
  dat_inv <- read.csv(paste0("../DS/2.5E/DS_inv_region_tables/", species, "_inv_region_table.csv"))
  glm_pred_inv_rst <- rast(paste0("../pred_rasters/invasive/", species, "_inv_p_glm.tif"))
  bart_pred_inv_rst <- rast(paste0("../pred_rasters/invasive/", species, "_inv_p_bart.tif"))
  fne_pred_inv_rst <- rast(paste0("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/Nf_modeling/Results-v3/", species, "_suitability_map_inv.tif"))
  ...
  eval_metrics_eur_mess[species, paste0(c("AUCratio", "pROCpval"), "_glm")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(glm_pred_inv_rst), threshold = 2.5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_eur_mess[species, paste0(c("AUCratio", "pROCpval"), "_bart")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(bart_pred_inv_rst), threshold = 2.5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_eur_mess[species, paste0(c("AUCratio", "pROCpval"), "_fne")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(fne_pred_inv_rst), threshold = 2.5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  ...
  eval_metrics_inv_mess[species, paste0(c("AUCratio", "pROCpval"), "_glm")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(glm_pred_inv_rst), threshold = 2.5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_inv_mess[species, paste0(c("AUCratio", "pROCpval"), "_bart")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(bart_pred_inv_rst), threshold = 2.5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_inv_mess[species, paste0(c("AUCratio", "pROCpval"), "_fne")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(fne_pred_inv_rst), threshold = 2.5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
...
dir.create("../DS/2.5E/DS_eval_metrics")

write.csv(mtp_thresholds, "../DS/2.5E/DS_eval_metrics/mtp_thresholds.csv", row.names = FALSE)

write.csv(data.frame(species = rownames(eval_metrics_eur_full), eval_metrics_eur_full), "../DS/2.5E/DS_eval_metrics/eval_metrics_eur_full.csv", row.names = FALSE)
write.csv(data.frame(species = rownames(eval_metrics_eur_mess), eval_metrics_eur_mess), "../DS/2.5E/DS_eval_metrics/eval_metrics_eur_mess.csv", row.names = FALSE)
write.csv(data.frame(species = rownames(eval_metrics_inv_full), eval_metrics_inv_full), "../DS/2.5E/DS_eval_metrics/eval_metrics_inv_full.csv", row.names = FALSE)
write.csv(data.frame(species = rownames(eval_metrics_inv_mess), eval_metrics_inv_mess), "../DS/2.5E/DS_eval_metrics/eval_metrics_inv_mess.csv", row.names = FALSE)
