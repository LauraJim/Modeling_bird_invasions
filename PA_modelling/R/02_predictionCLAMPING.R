...
dir.create("../pred_rastersCL/native", recursive = TRUE)
dir.create("../pred_rastersCL/invasive", recursive = TRUE)
dir.create("../pred_JPGsCL", recursive = TRUE)
...
for (species in species_list) {
  ...
  climgrids_inv <- stack(paste0("../../occ_dist_climgrids/", species, "/", species, "_ClimGridsPCA.invasive.tif"))
    spare.names <- names(climgrids_inv)
    names(climgrids_inv)<- c("PC1","PC2")
  
###ADJUST climgrids_inv FOR CLAMPING###
  sp.id <- read.csv("../../Nf_modeling/allspecies_samplesizes_v3.csv",header=T)[,c(1,4,6)]
  j<-species
  f.occ.nat <- paste0("../../Nf_modeling/accessible-areas-v3/",j,"_native_range.csv")
  sp.occ.nat <- read.csv(f.occ.nat,header=T)[,-1]
  
  climgrids_inv <- clamp.vars(climgrids_inv, sp.occ.nat[c(3:4)], left = NULL, right = NULL, categoricals = NULL)
  names(climgrids_inv) <- spare.names
  ...
  gc()
  bart_p_inv <- predict(mod_bart, climgrids_inv)
  ...
  writeRaster(glm_p_nat, paste0("../pred_rastersCL/native/", species, "_nat_p_glm.tif"))
  writeRaster(bart_p_nat, paste0("../pred_rastersCL/native/", species, "_nat_p_bart.tif"))
  writeRaster(glm_f_nat, paste0("../pred_rastersCL/native/", species, "_nat_f_glm.tif"))
  writeRaster(bart_f_nat, paste0("../pred_rastersCL/native/", species, "_nat_f_bart.tif"))
  
  writeRaster(glm_p_inv, paste0("../pred_rastersCL/invasive/", species, "_inv_p_glm.tif"))
  writeRaster(bart_p_inv, paste0("../pred_rastersCL/invasive/", species, "_inv_p_bart.tif"))
  writeRaster(glm_f_inv, paste0("../pred_rastersCL/invasive/", species, "_inv_f_glm.tif"))
  writeRaster(bart_f_inv, paste0("../pred_rastersCL/invasive/", species, "_inv_f_bart.tif"))
...
pred_files_nat <- list.files("../pred_rastersCL/native", full.names = TRUE)
pred_files_nat <- pred_files_nat[grep("_f_", pred_files_nat)]
pred_files_nat

pred_files_inv <- list.files("../pred_rastersCL/invasive", full.names = TRUE)
...
for (f in c(pred_files_nat, pred_files_inv)) {
  ...
  jpeg(paste0("../pred_JPGsCL/", filename, ".jpg"), width = 800, height = 430)
