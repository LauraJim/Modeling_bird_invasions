...
dir.create("../DS/2.5ECL/DS_inv_region_tables/")
dir.create("../DS/2.5ECL/DS_MESS_JPGs")
...
for (species in species_list) {
  ...
  glm_pred_inv_rst <- rast(paste0("../pred_rastersCL/invasive/", species, "_inv_p_glm.tif"))
  bart_pred_inv_rst <- rast(paste0("../pred_rastersCL/invasive/", species, "_inv_p_bart.tif"))
  fne_pred_inv_rst <- rast(paste0("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/Nf_modeling/Results-v3CL/", species, "_suitability_map_inv.tif"))
  ...
  jpeg(paste0("../DS/2.5E/DS_MESS_JPGs/", species, "_MESS.jpg"))
  ...
  write.csv(dat_inv, paste0("../DS/2.5ECL/DS_inv_region_tables/", species, "_inv_region_table.csv"), row.names = FALSE)
...
eval_metrics_eur_full <- eval_metrics_eur_mess <- eval_metrics_inv_full <- eval_metrics_inv_mess <- as.data.frame(matrix(NA, nrow = length(species_list), ncol = 5 * 3))
...
colnames(eval_metrics_eur_full) <- colnames(eval_metrics_eur_mess) <- colnames(eval_metrics_inv_full) <- colnames(eval_metrics_inv_mess) <- c("AUCratio_glm", "AUCratio_bart","AUCratio_fne", "pROCpval_glm", "pROCpval_bart","pROCpval_fne","omis_glm", "omis_bart","omis_fne","sens_glm", "sens_bart", "sens_fne","spec_glm", "spec_bart","spec_fne")
...
for (species in species_list) {
  ...
  occ_nat <- read.csv(paste0("../../Nf_modeling/occurrences-v3/", species, "_native.csv"))
  acc_nat <- read.csv(paste0("../../Nf_modeling/accessible-areas-v3/", species, "_native_range.csv"))
  glm_pred_nat_rst <- rast(paste0("../pred_rastersCL/native/", species, "_nat_p_glm.tif"))
  bart_pred_nat_rst <- rast(paste0("../pred_rastersCL/native/", species, "_nat_p_bart.tif"))
  fne_pred_nat_rst <- rast(paste0("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/Nf_modeling/Results-v3CL/", species, "_suitability_map_nat.tif"))
  ...
  glm_min_2.5_thresh <- quantile(glm_pred_occnat, probs = 0.05, na.rm = TRUE)
  bart_min_2.5_thresh <- quantile(bart_pred_occnat, probs = 0.05, na.rm = TRUE)
  fne_min_2.5_thresh <- quantile(fne_pred_occnat, probs = 0.05, na.rm = TRUE)
  ...
  dat_inv <- read.csv(paste0("../DS/2.5ECL/DS_inv_region_tables/", species, "_inv_region_table.csv"))
  glm_pred_inv_rst <- rast(paste0("../pred_rastersCL/invasive/", species, "_inv_p_glm.tif"))
  bart_pred_inv_rst <- rast(paste0("../pred_rastersCL/invasive/", species, "_inv_p_bart.tif"))
  fne_pred_inv_rst <- rast(paste0("D:/Ststrubb/OneDrive - UGent/Projects/MC_paper/Modeling_bird_invasions/Nf_modeling/Results-v3CL/", species, "_suitability_map_inv.tif"))
  ...
  eval_metrics_eur_mess[species, paste0(c("AUCratio", "pROCpval"), "_glm")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(glm_pred_inv_rst), threshold = 5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_eur_mess[species, paste0(c("AUCratio", "pROCpval"), "_bart")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(bart_pred_inv_rst), threshold = 5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_eur_mess[species, paste0(c("AUCratio", "pROCpval"), "_fne")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(fne_pred_inv_rst), threshold = 5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  ...
  eval_metrics_eur_mess[species, paste0(c("omis", "sens", "spec"), "_fne")] <- fne_threshmeasures
  
  message("evaluating on MESS invaded background...")
  bg <- dat_inv[dat_inv$inv_range == 1 & dat_inv$analog == 1, ]
  eval_metrics_inv_mess[species, paste0(c("AUCratio", "pROCpval"), "_glm")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(glm_pred_inv_rst), threshold = 5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_inv_mess[species, paste0(c("AUCratio", "pROCpval"), "_bart")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(bart_pred_inv_rst), threshold = 5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  eval_metrics_inv_mess[species, paste0(c("AUCratio", "pROCpval"), "_fne")] <- kuenm_proc(occ.test = bg[bg$presence == 1, c("lon", "lat")], model = raster(fne_pred_inv_rst), threshold = 5, rand.percent = 50, iterations = 1000)$pROC_summary  # or threshold = 2.5?
  ...
  eval_metrics_inv_mess[species, paste0(c("omis", "sens", "spec"), "_fne")] <- fne_threshmeasures
...
# export mtp_thresholds/2.5E/DS and eval metrics:

dir.create("../DS/2.5ECL/DS_eval_metrics")

write.csv(mtp_thresholds, "../DS/2.5ECL/DS_eval_metrics/mtp_thresholds.csv", row.names = FALSE)

write.csv(data.frame(species = rownames(eval_metrics_eur_full), eval_metrics_eur_full), "../DS/2.5ECL/DS_eval_metrics/eval_metrics_eur_full.csv", row.names = FALSE)
write.csv(data.frame(species = rownames(eval_metrics_eur_mess), eval_metrics_eur_mess), "../DS/2.5ECL/DS_eval_metrics/eval_metrics_eur_mess.csv", row.names = FALSE)
write.csv(data.frame(species = rownames(eval_metrics_inv_full), eval_metrics_inv_full), "../DS/2.5ECL/DS_eval_metrics/eval_metrics_inv_full.csv", row.names = FALSE)
write.csv(data.frame(species = rownames(eval_metrics_inv_mess), eval_metrics_inv_mess), "../DS/2.5ECL/DS_eval_metrics/eval_metrics_inv_mess.csv", row.names = FALSE)
