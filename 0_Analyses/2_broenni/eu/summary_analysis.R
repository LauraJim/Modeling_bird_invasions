...
data.niche_th_env <- read.table("E:/Modeling_bird_invasions/0_Analyses/2_broenni/eu/niche_results_th_env.txt",sep="\t",h=T)
  head(data.niche_th_env)
  colnames(data.niche_th_env)[c(1:5)] <- c('sim1','sim2','equ','niche_overlap_F','niche_overlap_T')
  head(data.niche_th_env)
...
#lines and points  
niche.dyn.exp <- ggplot(data.niche_th_env, aes(x=threshold, y=expansion.index, group=species.id)) +
    geom_line(aes(color=species.id))+
    geom_point(aes(color=species.id),size=3)+
    geom_point(aes(shape=species.id),size=3)+scale_shape_manual(values=seq(0,20)) +
    xlab("\n ") + ylab("niche expansion index") 
    niche.dyn.exp
  ggsave("niche.dyn.exp.png", plot=niche.dyn.exp, width=14, height=14, units="cm", dpi=1200)

niche.dyn.unf <- ggplot(data.niche_th_env, aes(x=threshold, y=unfilling.index, group=species.id)) +
    geom_line(aes(color=species.id))+
    geom_point(aes(color=species.id),size=3)+
    geom_point(aes(shape=species.id),size=3)+scale_shape_manual(values=seq(0,20))+
...
niche.dyn.stab <- ggplot(data.niche_th_env, aes(x=threshold, y=stability.index, group=species.id)) +
    geom_line(aes(color=species.id))+
    geom_point(aes(color=species.id),size=3)+
    geom_point(aes(shape=species.id),size=3)+scale_shape_manual(values=seq(0,20))+
...
niche.dyn.stab_vio <- ggplot(data.niche_th_env, aes(x=as.factor(threshold), y=stability.index)) + geom_violin()  
  ...
  setwd("E:/Modeling_bird_invasions/0_Analyses/2_broenni/eu")
...
niche.summary <- data.niche_th_env.75 %>%summarise(niche_overlap_F.D.mean = mean(niche_overlap_F),
                                       niche_overlap_F.D.sd = sd(niche_overlap_F),
                                       niche_overlap_F.T.mean = mean(niche_overlap_T),
                                       niche_overlap_F.T.sd = sd(niche_overlap_T),
  ...
  niche.summary
  write.table(niche.summary,"niche.summary_mean.txt",sep="\t",row.names=TRUE)
  
#median
  
  niche.summary <- data.niche_th_env.75 %>%summarise(niche_overlap_F.D.mean = median(niche_overlap_F),
                                                     niche_overlap_F.D.sd = sd(niche_overlap_F),
                                                     niche_overlap_F.T.mean = median(niche_overlap_T),
                                                     niche_overlap_F.T.sd = sd(niche_overlap_T),
                                                     stability.index.mean = median(stability.index),
                                                     stability.index.sd = sd(stability.index),
                                                     expansion.index.mean = median(expansion.index),
                                                     expansion.index.sd = sd(expansion.index),
                                                     unfilling.index.sd = sd(unfilling.index),
                                                     unfilling.index.mean = median(unfilling.index))
  niche.summary$sim1 <- sum(ifelse(data.niche_th_env.75$sim1>0.05,0,1))
  niche.summary$sim2 <- sum(ifelse(data.niche_th_env.75$sim2>0.05,0,1))
  niche.summary$equ <- sum(ifelse(data.niche_th_env.75$equ>0.05,0,1))
  niche.summary <- data.frame(t(niche.summary))
  colnames(niche.summary) <- "value"
  niche.summary
  write.table(niche.summary,"niche.summary_median.txt",sep="\t",row.names=TRUE)
