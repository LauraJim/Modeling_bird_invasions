...
###
  ...
  #model
  m_sens <- glmmTMB(sens ~ algorithm + (1|species),data_comp1, family=beta_family(link="logit"))
  m_sens <- lmer(sens ~ algorithm + (1|species),data_comp1)
    car::Anova(m_sens)
    pairs(emmeans(m_sens,  ~ algorithm))
    plot(allEffects(m_sens))
    
    #manual jittering of upper and lower data points
    upper <- data_comp1[data_comp1$sens >0.99,]
    ...
    lower <- data_comp1[data_comp1$sens <0.01,]
    ...
    other <- data_comp1[!data_comp1$sens <0.01,]
    other <- other[!other$sens >0.99,]
    data_comp1 <- rbind(lower, other,upper)
    nrow(data_comp1)
  
data_comp1$modelclass <- ifelse(grepl("sens",data_comp1$algorithm),'corr','mech')
    sens_cloud_plot <- ggplot(data_comp1, aes(x = algorithm, y = sens,fill=modelclass)) + scale_fill_manual(values=c("#E0E0E0", "#A0A0A0"))+
...
sens_cloud_plot <- sens_cloud_plot + 
    ...
    sens_cloud_plot 
    sens_cloud_plot2 <- sens_cloud_plot + geom_signif(comparisons = list(c("sens_fne", "NM_species")),y_position = 1.025,annotation = "",tip_length = 0.005,size=0.1)
    sens_cloud_plot2 <- sens_cloud_plot2 + geom_signif(comparisons = list(c("sens_fne", "NM_intra")),y_position = 1.05,annotation = "",tip_length = 0.005,size=0.1)
    sens_cloud_plot2 <- sens_cloud_plot2 + geom_signif(comparisons = list(c("sens_bart", "NM_intra")),y_position = 1.075,annotation = "",tip_length = 0.005,size=0.1)
    sens_cloud_plot2 <- sens_cloud_plot2 + geom_signif(comparisons = list(c("sens_glm", "NM_intra")),y_position = 1.1,annotation = "",tip_length = 0.005,size=0.1)
    sens_cloud_plot2 <- sens_cloud_plot2 +  scale_y_continuous(breaks=seq(0,1,0.2))
    sens_cloud_plot2 <- sens_cloud_plot2 + annotate("text", label = "A", x = 0.78, y = 1.1,size=3)
    sens_cloud_plot2
    
    ggsave(filename="./sens_cloud_plot.png",
...
sdm_comp2 <- eval_data_c[eval_data_c$invaded =="whole_europe",]
  ...
  #model
  m_sens <- glmmTMB(sens ~ algorithm + (1|species),data_comp2, family=beta_family(link="logit"))
  m_sens <- lmer(sens ~ algorithm + (1|species),data_comp2)
  car::Anova(m_sens)
  pairs(emmeans(m_sens,  ~ algorithm))
...
sdm_comp3 <- eval_data_c[eval_data_c$invaded =="dispersal",]
  ...
  #model
  m_sens <- glmmTMB(sens ~ algorithm + (1|species),data_comp3, family=beta_family(link="logit"))
  m_sens <- lmer(sens ~ algorithm + (1|species),data_comp3)
  car::Anova(m_sens)
  pairs(emmeans(m_sens,  ~ algorithm))
...
sdm_comp4 <- eval_data_c[eval_data_c$invaded =="dispersal",]
  ...
  #model
  m_sens <- glmmTMB(sens ~ algorithm + (1|species),data_comp4, family=beta_family(link="logit"))
  m_sens <- lmer(sens ~ algorithm + (1|species),data_comp4)
  car::Anova(m_sens)
  pairs(emmeans(m_sens,  ~ algorithm))
