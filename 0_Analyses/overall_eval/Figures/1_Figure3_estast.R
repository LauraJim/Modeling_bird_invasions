...
#mask
  ...
  #read in thresholds
  threshold_2.5E <- read.csv(paste0("../../1_PA_modelling/eval_metrics/NATIVE/BIAS/2.5E/crossval_metrics/crossval_metrics_nat_estast.csv"))
  
  #read in occurrence data
  occurrences <- read.csv(paste0("../../0_Nf_modeling/occurrences-v3/estast_invasive.csv"))
  ...
  p_glm5E <- ggplot() + geom_raster(data=glm_richness_2.5E_df,aes(x=x,y=y,fill=probs))+theme_void()+scale_fill_manual(values = "black")
  ...
  p_bart5E <- ggplot() + geom_raster(data=bart_richness_2.5E_df,aes(x=x,y=y,fill=probs))+theme_void()+scale_fill_manual(values = "black")
  ...
  ###NicheMapperSpecies
  #    
  nm_species <- raster(paste0("../../4_NicheMapper/","estast_yearly_energetics_pa.tif"))    
  ...
  ###################################################################################################################################    
  #get background map
  nativemap <- trim(raster(paste0("../../../1_Data/ranges/estast/nativemap.tif")))
  ...
  birdlife.raw<-readOGR(paste0("../../../1_Data/ranges/estast"),"estast")
  ...
  p.final <- p.final + annotate("text", label = "specificity: 0.00", x = 0.125, y = 0.955,size=2)
  
  p.final <- p.final + annotate("text", label = "sensitivity: 1.00", x = 0.625, y = 0.97,size=2)
  p.final <- p.final + annotate("text", label = "specificity: 0.00", x = 0.625, y = 0.955,size=2)
  
  
  p.final <- p.final + annotate("text", label = "sensitivity: 1.00", x = 0.12, y = 0.54,size=2)
  p.final <- p.final + annotate("text", label = "specificity: 0.34", x = 0.12, y = 0.525,size=2)
  
  
  p.final <- p.final + annotate("text", label = "sensitivity: 0.55", x = 0.122, y = 0.252,size=2)
  p.final <- p.final + annotate("text", label = "specificity: 0.95", x = 0.122, y = 0.237,size=2)
