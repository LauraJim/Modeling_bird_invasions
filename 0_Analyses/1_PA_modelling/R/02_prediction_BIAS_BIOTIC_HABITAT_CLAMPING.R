...
dir.create("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/native", recursive = TRUE)
dir.create("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/EU/invasive", recursive = TRUE)
dir.create("../pred_JPGsBIAS_BIOTIC_HABITAT_CLAMPING", recursive = TRUE)
...
foreach (species = species_list,.packages=c('terra','embarcadero','fuzzySim','modEvA','maps','ENMeval'))  %dopar% {
  ...
  ###ADJUST climgrids_inv FOR CLAMPING###
    sp.id <- read.csv("../../0_Nf_modeling/allspecies_samplesizes_v3.csv",header=T)[,c(1,4,6)]
      ...
      sp.occ.nat <- raster::extract(predictorgrids_nat,sp.occ.nat[c(1:2)])
      head(sp.occ.nat)

      #climgrids_for_clamping <- stack(predictorgrids_inv[[1]],predictorgrids_inv[[2]])
      #clamped_climgrids <- clamp.vars(climgrids_for_clamping, sp.occ.nat, left = NULL, right = NULL, categoricals = NULL)
      #predictorgrids_inv <- stack(clamped_climgrids,predictorgrids_inv[[3]],predictorgrids_inv[[4]],predictorgrids_inv[[5]],predictorgrids_inv[[6]])
      
      predictorgrids_inv <- clamp.vars(predictorgrids_inv, sp.occ.nat, left = NULL, right = NULL, categoricals = NULL)
      
  #introduce zero bias layers  
  ...
  writeRaster(glm_p_nat, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/native/", species, "_nat_p_glm.tif"),overwrite=TRUE)
  writeRaster(bart_p_nat, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/native/", species, "_nat_p_bart.tif"),overwrite=TRUE)
  writeRaster(glm_f_nat, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/native/", species, "_nat_f_glm.tif"),overwrite=TRUE)
  writeRaster(bart_f_nat, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/native/", species, "_nat_f_bart.tif"),overwrite=TRUE)
  
  writeRaster(glm_p_inv, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/EU/invasive/", species, "_inv_p_glm.tif"),overwrite=TRUE)
  writeRaster(bart_p_inv, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/EU/invasive/", species, "_inv_p_bart.tif"),overwrite=TRUE)
  writeRaster(glm_f_inv, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/EU/invasive/", species, "_inv_f_glm.tif"),overwrite=TRUE)
  writeRaster(bart_f_inv, paste0("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/EU/invasive/", species, "_inv_f_bart.tif"),overwrite=TRUE)
...
pred_files_nat <- list.files("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/native", full.names = TRUE)
pred_files_nat <- pred_files_nat[grep("_f_", pred_files_nat)]
pred_files_nat

pred_files_inv <- list.files("../pred_rastersBIAS_BIOTIC_HABITAT_CLAMPING/EU/invasive", full.names = TRUE)
...
for (f in c(pred_files_nat, pred_files_inv)) {
  ...
  jpeg(paste0("../pred_JPGsBIAS_BIOTIC_HABITAT_CLAMPING/", filename, ".jpg"), width = 800, height = 430)
