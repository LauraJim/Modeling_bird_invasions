...
dir.create("../pred_rastersBIAS/native", recursive = TRUE)
dir.create("../pred_rastersBIAS/EU/invasive", recursive = TRUE)
dir.create("../pred_JPGsBIAS", recursive = TRUE)
...
foreach (species = species_list,.packages=c('terra','embarcadero','fuzzySim','modEvA','maps'))  %dopar% {
  ...
  writeRaster(glm_p_nat, paste0("../pred_rastersBIAS/native/", species, "_nat_p_glm.tif"),overwrite=TRUE)
  writeRaster(bart_p_nat, paste0("../pred_rastersBIAS/native/", species, "_nat_p_bart.tif"),overwrite=TRUE)
  writeRaster(glm_f_nat, paste0("../pred_rastersBIAS/native/", species, "_nat_f_glm.tif"),overwrite=TRUE)
  writeRaster(bart_f_nat, paste0("../pred_rastersBIAS/native/", species, "_nat_f_bart.tif"),overwrite=TRUE)
  
  writeRaster(glm_p_inv, paste0("../pred_rastersBIAS/EU/invasive/", species, "_inv_p_glm.tif"),overwrite=TRUE)
  writeRaster(bart_p_inv, paste0("../pred_rastersBIAS/EU/invasive/", species, "_inv_p_bart.tif"),overwrite=TRUE)
  writeRaster(glm_f_inv, paste0("../pred_rastersBIAS/EU/invasive/", species, "_inv_f_glm.tif"),overwrite=TRUE)
  writeRaster(bart_f_inv, paste0("../pred_rastersBIAS/EU/invasive/", species, "_inv_f_bart.tif"),overwrite=TRUE)
...
pred_files_nat <- list.files("../pred_rastersBIAS/native", full.names = TRUE)
pred_files_nat <- pred_files_nat[grep("_f_", pred_files_nat)]
pred_files_nat

pred_files_inv <- list.files("../pred_rastersBIAS/EU/invasive", full.names = TRUE)
...
for (f in c(pred_files_nat, pred_files_inv)) {
  ...
  jpeg(paste0("../pred_JPGsBIAS/", filename, ".jpg"), width = 800, height = 430)
