...
dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/CV_blocksBIAS_BIOTIC_HABITAT")
dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/pred_CSVsBIAS_BIOTIC_HABITAT")
...
foreach (species = species_list,.packages=c('terra','embarcadero','fuzzySim','modEvA','kuenm','blockCV'))  %dopar% {
  ...
  predictorvars <- stack(PC1,PC2,bias,PCbiotic1,PCbiotic2,PChabitat1,PChabitat2)
  plot(predictorvars)
  names(predictorvars) <- var_names <- c("PC1", "PC2","bias","PCbiotic1","PCbiotic2","PChabitat1","PChabitat2")
  ...
  jpeg(paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/CV_blocksBIAS_BIOTIC_HABITAT/", species, ".jpg"), width = 500, height = 500)
  ...
  var_names <- c("PC1", "PC2","bias","PCbiotic1","PCbiotic2","PChabitat1","PChabitat2")
  ...
  # save result to disk:
  write.csv(pa_nat, paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/pred_CSVsBIAS_BIOTIC_HABITAT/", species, "_pred_crossval.csv"), row.names = FALSE)
...
#blocks
#saveRDS(blocks, "../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/CV_blocksBIAS_BIOTIC_HABITAT/_blocks.rds")

remove(cl)

# COMPUTE CROSS-VALIDATION METRICS ####

dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/crossval_boxplots", recursive = TRUE)
dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/crossval_metrics", recursive = TRUE)
...
foreach (species = species_list,.packages=c('terra','embarcadero','fuzzySim','modEvA','kuenm'))  %dopar% {
  pred_nat <- read.csv(paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/pred_CSVsBIAS_BIOTIC_HABITAT/", species, "_pred_crossval.csv"))
  ...
  write.csv(temp, paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/crossval_metrics/crossval_metrics_nat_", species, ".csv"), row.names = FALSE)
  
  # boxplots of the cross-validation metrics:
  jpeg(paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/5E/crossval_boxplots/", species, ".jpg"))
  par(mar = c(7.2, 3, 2, 1))
...
dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/CV_blocksBIAS_BIOTIC_HABITAT")
dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/pred_CSVsBIAS_BIOTIC_HABITAT")
...
foreach (species = species_list,.packages=c('terra','embarcadero','fuzzySim','modEvA','kuenm','blockCV'))  %dopar% {
  ...
  # plot spatial blocks with occ data:
  jpeg(paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/CV_blocksBIAS_BIOTIC_HABITAT/", species, ".jpg"), width = 500, height = 500)
  plot(trim(climgrids_nat[[1]]), xlim = ext(blocks[[species]]$blocks)[1:2], ylim = ext(blocks[[species]]$blocks)[3:4], col = "wheat2", legend = FALSE, main = species, cex.main = 2)
  plot(blocks[[species]]$blocks, border = "grey40", add = TRUE)
  points(pa_nat[pa_nat$presence == 0, c("lon", "lat")], pch = 20, cex = 0.1, col = "salmon")
  text(blocks[[species]]$blocks, blocks[[species]]$blocks$folds, col = blocks[[species]]$blocks$folds, halo = TRUE)  # col = c("black", "blue", "red", "darkgreen", "purple")
  ...
  # save result to disk:
  write.csv(pa_nat, paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/pred_CSVsBIAS_BIOTIC_HABITAT/", species, "_pred_crossval.csv"), row.names = FALSE)
...
#blocks
#saveRDS(blocks, "../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/CV_blocksBIAS_BIOTIC_HABITAT/_blocks.rds")

remove(cl)

# COMPUTE CROSS-VALIDATION METRICS ####

dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/crossval_boxplots", recursive = TRUE)
dir.create("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/crossval_metrics", recursive = TRUE)
...
foreach (species = species_list,.packages=c('terra','embarcadero','fuzzySim','modEvA','kuenm'))  %dopar% {
  pred_nat <- read.csv(paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/pred_CSVsBIAS_BIOTIC_HABITAT/", species, "_pred_crossval.csv"))
  ...
  temp <- data.frame(fold = as.integer(rownames(crossval)), crossval)
  write.csv(temp, paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/crossval_metrics/crossval_metrics_nat_", species, ".csv"), row.names = FALSE)
  
  # boxplots of the cross-validation metrics:
  jpeg(paste0("../eval_metrics/NATIVE/BIAS_BIOTIC_HABITAT/2.5E/crossval_boxplots/", species, ".jpg"))
  par(mar = c(7.2, 3, 2, 1))
  boxplot(crossval[ , 3:ncol(crossval)], las = 2, main = species)
  abline(h = 1, col = "darkgrey", lty = 2)
  dev.off()
