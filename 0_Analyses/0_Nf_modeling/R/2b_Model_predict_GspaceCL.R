...
for (k in 1:nrow(mles)) {
  ...
  suit.wn.nat <- niche.G(Estck = pc.native, mu = c(mles[k,3],mles[k,4]), 
                       Sigma = matrix(c(mles[k,5], mles[k,6], mles[k,6], 
                                        mles[k,7]),ncol=2))
  writeRaster(suit.wn.nat,paste0("../../../0_Analyses/0_Nf_modeling/ResultsClimateClamping/",species.id,
  ...
  j<-species.id
  f.occ.nat <- paste0("../../../0_Analyses/0_Nf_modeling/accessible-areas-v3/",j,"_native_range.csv")
  ...
  writeRaster(suit.wn.inv,paste0("../../../0_Analyses/0_Nf_modeling/ResultsClimateClamping/",species.id,
