...
# Read species IDs
sp.id <- read.csv("../../0_Nf_modeling/allspecies_samplesizes_ClimBioticHabitat.csv",header=T)[,c(1,4,6)]
...
for (j in 1:nrow(sp.id)) {
  ...
  f.occ.nat <- paste0("../../0_Nf_modeling/occurrencesClimBioticHabitat/",sp.id[j,1],"_native.csv")
  sp.occ.nat <- read.csv(f.occ.nat,header=T)[,-1]
  # invasive range, used to evaluate the models
  f.occ.inv <- paste0("../../0_Nf_modeling/occurrencesClimBioticHabitat/",sp.id[j,1],"_invasive.csv")
  sp.occ.inv <- read.csv(f.occ.inv,header=T)[,-1]
  
  # 3) Read tables with random samples in the native and invaded areas
  # native range, used to fit the models
  f.rs.nat <- paste0("../../0_Nf_modeling/accessible_areas_ClimBioticHabitat/",sp.id[j,1],
                     "_native_range.csv")
  sp.rs.nat <- read.csv(f.rs.nat,header=T)[,-1]
  # invasive range, used to evaluate the models
  f.rs.inv <- paste0("../../0_Nf_modeling/accessible_areas_ClimBioticHabitat/",sp.id[j,1],
                     "_invaded_region.csv")
  ...
  if(mle$dp == 1){ # Sigma is positive definite
    ...
    png(paste0("../../0_Nf_modeling/ResultsClimBioticHabitat/",sp.id[j,1],"_modelfit.png"),
  ...
  if(j==nrow(sp.id))
    write.csv(mle.summary,"../../0_Nf_modeling/ResultsClimBioticHabitat/mle_allspecies_v3.csv",row.names = F)
  #print(mle.summary)
}
